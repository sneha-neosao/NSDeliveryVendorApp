...
class RemoteDataSourceImpl implements RemoteDataSource {
  ...
  @override
  Future<ProfileResponse> ProfileList(String token) async {
    try {
      final response = await _helper.execute(
        method: Method.get,
        url: ApiUrl.profileList,
        options: Options(
          headers: {
            'Authorization': 'Bearer $token',
          },
  ...
  @override
  Future<PerformanceMetricsResponse> DashboardPerformanceMetrics(
      String token) async {
    try {
      final response = await _helper.execute(
        method: Method.get,
        url: ApiUrl.dashboardPerformanceMetrics,
        options: Options(
          headers: {
            'Authorization': 'Bearer $token',
          },
  ...
  @override
  Future<OffersListResponse> OffersList(
      String token, OffersListParams params) async {
    try {
      final queryString = 'page=${params.page}&limit=${params.limit}';

      final response = await _helper.execute(
        method: Method.get,
        url: '${ApiUrl.offersList}?$queryString',
        options: Options(
          headers: {
            'Authorization': 'Bearer $token',
          },
